...
/// Resolve a wikilink target to a real .md file path. Thin shell wrapper over
/// `markdown_core::wikilinks::resolve` (EPIC-CORE-API).
#[tauri::command]
fn resolve_wikilink(link_text: String, current_file_path: String) -> Option<String> {
    markdown_core::wikilinks::resolve(&link_text, &current_file_path)
}

/// Serializable backlink DTO for the editor frontend — mirrors
/// `markdown_core::wikilinks::Backlink`.
...
impl From<markdown_core::wikilinks::Backlink> for BacklinkEntry {
    fn from(b: markdown_core::wikilinks::Backlink) -> Self {
        BacklinkEntry {
            path: b.path,
            line: b.line,
            context: b.context,
...
/// Compute backlinks for the given file. Thin shell wrapper over
/// `markdown_core::wikilinks::backlinks` (EPIC-CORE-API).
#[tauri::command]
fn compute_backlinks(file_path: String) -> Result<Vec<BacklinkEntry>, String> {
    Ok(markdown_core::wikilinks::backlinks(&file_path)?
        .into_iter()
        .map(BacklinkEntry::from)
        .collect())
}

/// Create a new .md file for a wikilink target that doesn't exist yet. Thin
/// shell wrapper over `markdown_core::wikilinks::create_target` (EPIC-CORE-API).
#[tauri::command]
fn create_wikilink_target(link_text: String, current_file_path: String) -> Result<String, String> {
    markdown_core::wikilinks::create_target(&link_text, &current_file_path)
}
